...
const isModeType = computed(() => {
    return (
        currentFileEdit.type === 'mode' || currentFileEdit.type === 'modes' || currentFileEdit.type === 'generalMode'
    );
});
    ...
    window.dialogApi
        ...
        .then((result: any) => {
            if (!result.canceled) {
                ...
                console.debug(`选择的封面图片路径: ${filePath}`);
...
const saveEdit = () => {
    ...
    window.fsApi.writeFile(`${currentFile.path}/fileData.json`, JSON.stringify(currentFile, null, 4), (err: any) => {
        ...
        console.debug(`数据修改成功: `, currentFileEdit);
        emit('confirmEdit');
    });
};
            ...
            <div class="dialog__body-content">
                <div class="dialog__body-content-title">
                    ...
                    <div class="dialog__body-content-type">
                        <template
                            v-for="(item, index) in Object.keys(fileType).filter((v) => v !== 'firstFolder')"
                            :key="index"
                        >
                            <span
                                class="dialog__body-content-type-item"
                                :class="{ active: item === currentFileEdit.type }"
                                @click.stop="changeType(item)"
                                >{{ fileType[item] }}</span
                            ><span v-if="index + 1 !== Object.keys(fileType).filter((v) => v !== 'firstFolder').length"
                                >/</span
                            >
                        </template>
                    </div>
                ...
                <div
                    class="dialog__body-content-left"
                    :style="{ 'background-image': `url(${filterImgPath(currentFileEdit.cover)})` }"
                >
                    <div class="dialog__body-content-left-bottom">
                        <div class="dialog__body-content-left-bottom-preview" @click.stop="showPicturePreview">
                            预览大图
                        </div>
                ...
                <div class="dialog__body-content-right">
                    <div class="dialog__body-form-item">
                        ...
                        <div class="dialog__body-form-item-operate dialog__body-form-item-operate_name">
                            {{ currentFileEdit.fileName }}
                        </div>
                    </div>
                    <div class="dialog__body-form-item" v-if="isModeType || currentFileEdit.type === 'firstFolder'">
                        <div class="dialog__body-form-item-label">
                            {{
                                `${
                                    currentFileEdit.type === 'firstFolder'
                                        ? '目录'
                                        : currentFileEdit.type === 'mode'
                                        ? '角色'
                                        : 'mode'
                                }名称`
                            }}
                        ...
                        <div class="dialog__body-form-item-operate">
                            <input
                                class="dialog__body-form-item-input"
                                style="text-align: center;"
                                v-model="currentFileEdit.name"
                                type="text"
                            />
                        </div>
                    </div>
                    ...
                    <div class="dialog__body-form-item" v-if="currentFileEdit.type === 'mode'">
                        ...
                        <div class="dialog__body-form-item-right">
                            <div
                                class="dialog__body-form-item-key"
                                v-for="(key, keyIndex) in currentFileEdit.keys"
                                :key="keyIndex"
                                :title="key[1]"
                            >
                                {{ key[0] }}
                            ...
                            <div class="dialog__body-form-item-operate dialog__body-form-item-operate_key">
                                <div
                                    class="dialog__body-form-item-btn"
                                    :style="{ transform: `rotate(${addKeyShow ? '0deg' : '90deg'})` }"
                                    @click.stop="changeAddKey"
                                >
                        ...
                        <div class="dialog__body-form-item-keys" v-if="addKeyShow">
                            ...
                            <div
                                class="dialog__body-form-item-keys-item"
                                v-for="(key, keyIndex) in currentFileEditKeys"
                                :key="keyIndex"
                            >
